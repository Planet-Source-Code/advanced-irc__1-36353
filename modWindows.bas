...
Sub ConnectNewStatusWnd()
    With IRCInfo
        If .AutoMode = 1 Then 'Show option dialog
            NewStatusWnd
            ShowOptionWnd
            If StatusWnd(StatusWndU).IRC.State = 0 Then 'User did not connect
...
Sub NewStatusWnd(Optional ByVal Server As String, Optional ByVal Port As String, Optional ByVal First As Boolean)
    ...
    If RemoteConnected Then frmMain.SendRemote "OPEN"
...
Sub UnloadStatusWnd(ByVal tServerNum As Integer, Optional ByVal UnloadMode As Integer = 0)
    ...
    If RemoteConnected Then frmMain.SendRemote "KILL " & tServerNum
...
Sub NewChatWnd(ByVal Nick As String, ByVal IP As String, ByVal Port As String, ByVal DoConnect As Boolean, Optional ByVal UID As Long)
    ...
    With ChatWnd(ChatWndU)
        ...
        If DoConnect Then 'Is client
            .UniqueID = UID
            If Not UID = 0 Then
                Output "*** " & Nick & " wants to start a passive DCC chat.", ChatWnd(ChatWndU), ColorInfo.cStatus
                .DCCProtocol = dccPassive
            Else
                Output "*** " & Nick & " wants to start a DCC chat (" & IP & ":" & Port & ")", ChatWnd(ChatWndU), ColorInfo.cStatus
                .Chat.RemoteHost = IP
                .Chat.RemotePort = Port
            End If
            Output "*** Press Enter to accept, Escape to decline", ChatWnd(ChatWndU), ColorInfo.cStatus
        Else 'Is server
            If DCCInfo.PassiveDCC Then 'Initalize passive DCC
                .DCCProtocol = dccPassive
                Inc DCCUnique
                .UniqueID = DCCUnique
                Output "*** Initializing passive DCC, waiting for acknowledgment...", ChatWnd(ChatWndU), ColorInfo.cStatus
                SendCTCP Nick, "DCC CHAT " & StatusWnd(ActiveServer).CurrentNick & " " & PutIP(DCCIP) & " 0 " & .UniqueID, True
            Else
                .Chat.Bind NextDCCPort, DCCIP
                .Chat.Listen
                Output "*** Waiting for reply...", ChatWnd(ChatWndU), ColorInfo.cStatus
...
Sub NewDCCWnd(ByVal Nick As String, ByVal FileName As String, ByVal Size As Currency, ByVal IP As String, _
              ...
              Optional ByVal UID As Long = 0, Optional ByVal IsUDP As Boolean = False)
    Dim V As Variant
    Dim Fn As Integer
    Inc DCCWndU
    ...
    With airc_AddIns(airc_AddInCount)
        ...
        If Not S = "" Then
            ...
            For C = LBound(V) To UBound(V)
                Output "PLUGIN> (" & .AddinName & ") " & V(C), fActive, ColorInfo.cStatus, True
            Next
        End If
    End With
End Sub
