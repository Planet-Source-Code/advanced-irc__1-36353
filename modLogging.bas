...
Sub LogStr(F As Form, ByVal Text As String)
    ...
    Exit Sub
ErrHandle:
    Output "Could not write to log.", fActive, ColorInfo.cStatus, True, True
    Resume Next
End Sub
...
Sub OpenLog(L As LogTypes, Optional N As String)
    ...
    Exit Sub
ErrHandle:
    Output "Could not open log for " & N & "!", fActive, ColorInfo.cStatus, True, True
    On Error GoTo 0
    Err.Clear
End Sub
...
Sub CloseLog(L As LogTypes, Optional N As String)
    ...
    Exit Sub
ErrHandle:
    'Output "Could not close log for " & N & "!", fActive, ColorInfo.cStatus, True, True
